...
{
    public partial class frmDriverRentTransactionExpensesReport3 : UI.SetupBase
    {
        ...
        public frmDriverRentTransactionExpensesReport3(int val)
        {
            InitializeComponent();
        ...
        public frmDriverRentTransactionExpensesReport3(IList LIST, DateTime from, DateTime till)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                if (grdLister.Rows.Count > 0)
                {
                    ...
                    if (row != null)
                    {
                        ...
                        SetDataSourceAndGenerateReport(row.Cells["RentId"].Value.ToInt(), row.Cells["VAT"].Value.ToDecimal());
                    }
        ...
        public bool SendEmailInternally(frmEmail frmE, string subject, string invoiceNo, string email)
        {
            ...
            if (subject.ToStr().Contains("||"))
            {
                try
                {

                    var arr = subject.ToStr().Split(new string[] { "||" }, StringSplitOptions.RemoveEmptyEntries);

                    subject = arr[0];
                    frmE.txtBody.Text = arr[1];
        ...
        private void optFullDetail_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
