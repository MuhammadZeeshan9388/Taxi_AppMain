...
{
    public partial class rptfrmJobsListVehicle : UI.SetupBase
    {
        public rptfrmJobsListVehicle()
        {
            InitializeComponent();
        ...
        private void rptfrmJobsList_Load(object sender, EventArgs e)
        {
         ...
         //   (grdLister.Columns[COLS.BookingDate] as GridViewDateTimeColumn).CustomFormat = "dd/MM/yyyy HH:mm";
          //  (grdLister.Columns[COLS.BookingDate] as GridViewDateTimeColumn).FormatString = "{0:dd/MM/yyyy HH:mm}";
            ...
            {
                grdLister.Columns[COLS.Notes].IsVisible = true;
                // grdLister.Columns[COLS.BookedBy].IsVisible = false;
                grdLister.Columns[COLS.InvoiceNo].IsVisible = false;
                grdLister.Columns[COLS.DespatchBy].IsVisible = false;
             ...
             //   grdLister.CellEndEdit += new GridViewCellEventHandler(grdLister_CellEndEdit);
                ...
                //if (AppVars.listUserRights.Count(c => c.formName == this.Name && (c.functionId == "RESET AGENT COMMISSION")) > 0)
                //{
            ...
            try
            {
                ...
                if (gridCell.ColumnInfo.Name == "btnUpdate")
                {
                    ...
                    if (row is GridViewDataRowInfo)
                    {
                            ...
                            if (AppVars.listUserRights.Count(c => c.formName == "frmBooking" && (c.functionId == "LOCK BOOKING AFTER TRANSACTION"")) > 0)
                            {
                                ...
                                using (TaxiDataContext db = new TaxiDataContext())
                                {
                                    if (db.Invoice_Charges.Count(c => c.BookingId == id) > 0)
                                    {
                                        ...
                                        reasonCnt++;
                                        error = reasonCnt + ". Invoice Exist" + Environment.NewLine;
                                    }
                        ...
                        {
                            if (General.GetQueryable<Gen_Company_PaymentType>(c => c.CompanyId == accountId && c.PaymentTypeId == paymentTypeId).Count() == 0)
                            {
                        ...
                        if (objMaster.Current != null)
                        {
                            ...
                            if (objMaster.Current.CompanyId != null)
                            {
                                ...
                                objMaster.Current.TotalCharges = objMaster.Current.CompanyPrice + objMaster.Current.ParkingCharges.ToDecimal() + objMaster.Current.WaitingCharges.ToDecimal();
                            }
                            else
                            {

                                objMaster.Current.TotalCharges = objMaster.Current.FareRate + objMaster.Current.MeetAndGreetCharges.ToDecimal() + objMaster.Current.CongtionCharges.ToDecimal();
        ...
        private List<Vu_BookingBase> GetDataSource(int reportType, int companyId, DateTime? fromDate, DateTime? toDate, int FleetMasterId, int PaymentTypeId, string BookedBy, int CompanyGroupId)
        {
                ...
                return General.GetQueryable<Vu_BookingBase>(c =>
                                     (
                                        ...
                                        (reportType == eReportType.CANCELLED_JOBS && c.BookingStatusId == Enums.BOOKINGSTATUS.CANCELLED) ||
                                         (reportType == eReportType.ALL_JOBS && c.BookingStatusId != null)

                                     )

                                     && (companyId == 0 || c.CompanyId == companyId)
                                     && (FleetMasterId == 0 || c.FleetMasterId == FleetMasterId)
                                     && (PaymentTypeId == 0 || (listofPaymentTypes != null && listofPaymentTypes.Contains(c.PaymentTypeId)))
                                     && (CompanyGroupId == 0 || c.CompanyGroupId == CompanyGroupId)
                              && ((fromDate == null || (fullfromdateCriteria && c.PickupDateTime.Value >= fromDate) || (fullfromdateCriteria == false && c.PickupDateTime.Value.Date >= fromDate))
                              && (toDate == null || (fulltodateCriteria && c.PickupDateTime.Value <= toDate) || (fulltodateCriteria == false && c.PickupDateTime.Value.Date <= toDate)))
                              && (c.SubCompanyId == AppVars.DefaultBookingSubCompanyId || AppVars.DefaultBookingSubCompanyId == 0)
                              && (c.OrderNo == orderNo || orderNo == string.Empty)
        ...
        public override void Print()
        {
            try
            {
                ...
                frm.DataSource = GetDataSource(GetReportType(), ddlCompany.SelectedValue.ToInt(), fromDate, toDate, ddlAllVehicle.SelectedValue.ToInt(), ddlPaymentType.SelectedValue.ToInt(), ddlBookedBy.Text.Trim(), ddlCompanyGroup.SelectedValue.ToInt());
        ...
        private void ViewReport()
        {
            ...
            try
            {
                ...
                var list = GetDataSource(GetReportType(), ddlCompany.SelectedValue.ToInt(), dtpFromDate.Value.ToDateorNull(), dtpToDate.Value.ToDateorNull(), ddlAllVehicle.SelectedValue.ToInt(), ddlPaymentType.SelectedValue.ToInt(), BookedBy, ddlCompanyGroup.SelectedValue.ToInt());
        ...
        private void chkAllDriver_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                ddlAllVehicle.SelectedValue = null;
                ddlAllVehicle.Enabled = false;
                //ddlAllDriver.SelectedValue = 0;
            }

            else
            {
                if (ddlAllVehicle.DataSource == null)
                {
                    //ComboFunctions.FillDriverCombo(ddlAllDriver);
                    ComboFunctions.FillVehicleCombo(ddlAllVehicle);
                }
                ddlAllVehicle.Enabled = true;
            }
        }
