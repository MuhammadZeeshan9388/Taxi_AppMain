...
{
    public partial class frmDriverRentLastStatement2 : UI.SetupBase
    {
        public frmDriverRentLastStatement2()
        {
            InitializeComponent();
            FormatGrid();
        ...
        private void PrintStatement()
        {
            try
            {
                ...
                if (grdDriverRentLastStatement.Rows.Where(c => c.Cells["Check"].Value.ToBool() == true).Count() > 0)
                {
                    ...
                    if (list.Count > 0)
                    {
                        frmDriverRentTransactionExpensesReport2 frm = new frmDriverRentTransactionExpensesReport2(list, DateTime.Now.ToDate(), DateTime.Now.ToDate());
                        frm.ShowDialog();
                        frm.Dispose();
                    }
