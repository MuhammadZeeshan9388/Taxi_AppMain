    ...
    public partial class frmAdvanceBookingCustomization : RadForm
    {
             ...
             //       && c.BookingStatusId != Enums.BOOKINGSTATUS.CANCELLED && c.MasterJobId==null )
                ...
                for (int i = 0; i < rowCnt; i++)
                {
                    if (i == 0)
                    {
                        ...
                        dtpDayWisePickupTime.Value = bookinglist[i].PickupDateTime;

                        dtpStartingAt.Value = bookinglist[i].PickupDateTime;
                        dtpStartingCancel.Value = bookinglist[i].PickupDateTime;
                        ...
                        txtCustomerName.Text = bookinglist[i].CustomerName.ToStr();
                        txtEmail.Text = bookinglist[i].CustomerEmail.ToStr();
                        txtMobileNo.Text = bookinglist[i].CustomerMobileNo.ToStr();
                        txtTelephoneNo.Text = bookinglist[i].CustomerPhoneNo.ToStr();
                        num_TotalPassengers.Value = bookinglist[i].NoofPassengers.ToDecimal();
                        numTotalLuggages.Value = bookinglist[i].NoofLuggages.ToDecimal();
                        numCustomerPrice.Value = bookinglist[i].CustomerPrice.ToDecimal();
                        numCompanyPrice.Value = bookinglist[i].CompanyPrice.ToDecimal();
                        numPickupFares.Value = bookinglist[i].FareRate.ToDecimal();

                        ddlPaymentType.SelectedValue = bookinglist[i].PaymentTypeId.ToInt();
                        ...
                        ddlAccount.SelectedValue = bookinglist[i].CompanyId;
                                    
                        ddlVehicle.SelectedValue = bookinglist[i].VehicleTypeId.ToInt();

                        txtOrderNo.Text = bookinglist[i].OrderNo.ToStr();


                        if (bookinglist[i].JourneyTypeId.ToInt() == Enums.JOURNEY_TYPES.ONEWAY)
                        {
                            txtSpecialReq.Text = bookinglist[i].SpecialRequirements.ToStr();
                            numCustomerPrice.Value = bookinglist[i].CustomerPrice.ToDecimal();
                            numCompanyPrice.Value = bookinglist[i].CompanyPrice.ToDecimal();
                        }
                        if (bookinglist[i].JourneyTypeId.ToIntorNull() == Enums.JOURNEY_TYPES.RETURN)
                        {
                            lblReturnSpecialReq.Visible = true;
                            txtReturnSpecialRequirement.Visible = true;
                            txtReturnSpecialRequirement.Text = bookinglist[i].SpecialRequirements.ToStr();
                            numReturnCustomerFare.Value = bookinglist[i].CustomerPrice.ToDecimal();
                            numReturnCompanyPrice.Value = bookinglist[i].CompanyPrice.ToDecimal();
                            numReturnFares.Value = bookinglist[i].ReturnFareRate.ToDecimal();
                            dtpDayWiseReturnPickUpTime.Enabled = true;
                            dtpDayWiseReturnPickUpTime.Value = bookinglist[i].ReturnPickupDateTime.ToDateTime();
                        }
                        ...
                        if (ShowVia == false)
                        {
                            ...
                            DisplayBooking_ViaLocations(bookinglist[i].Booking_ViaLocations);
                            ShowVia = true;
                        }
                    }
                    ...
                    else if (i == (rowCnt - 1))
                    {
                        ...
                        dtpEndingCancel.Value = bookinglist[i].PickupDateTime;
                    }
                    ...
                    grdBookings.Rows[i].Cells[COLS.BookingStatus].Value = bookinglist[i].BookingStatusId;
                    ...
                    if (bookinglist[i].ReturnPickupDateTime != null)
                    {
                        ...
                        grdBookings.Rows[i].Cells[COLS.ReturnPickupDate].Value = string.Format("{0:dd/MM/yyyy HH:mm}", bookinglist[i].ReturnPickupDateTime);                     
        ...
        public void UpdateBooking()
        {
            try
            {
                ...
                //booking.DisableUpdateReturnJob = true;
              //  objMaster.DisableUpdateReturnJob = true;
                ...
                //{
                //    //Booking objBooking = (from p in db.Bookings
                //    //where p.Id==Id
                //    //select p).SingleOrDefault();


                //    //objBooking.FareRate = grdBookings.CurrentRow.Cells[COLS.Fare].Value.ToDecimal();
                //    //objBooking.ReturnFareRate = grdBookings.CurrentRow.Cells[COLS.ReturnFare].Value.ToDecimal();
                //    //db.SubmitChanges();
                //    //Booking objBooking = new Booking();
                //    //Booking objBooking = UDB.Users.Single(u => u.Id == user.Id);
                //    Booking objBooking = db.Bookings.Single(a => a.Id == Id);
                //    objBooking.FareRate = grdBookings.CurrentRow.Cells[COLS.Fare].Value.ToDecimal();
                //    objBooking.ReturnFareRate = grdBookings.CurrentRow.Cells[COLS.ReturnFare].Value.ToDecimal();


                //    objBooking.PickupDateTime = grdBookings.CurrentRow.Cells[COLS.PickupDate].Value.ToDateTimeorNull();
                ...
                //    if (FromLocTypeId == Enums.LOCATION_TYPES.ADDRESS || FromLocTypeId == Enums.LOCATION_TYPES.BASE)
                //        objBooking.FromAddress = grdBookings.CurrentRow.Cells[COLS.FromAddress].Value.ToStr();

                //    else if (FromLocTypeId == Enums.LOCATION_TYPES.POSTCODE)
                //        objBooking.FromAddress = grdBookings.CurrentRow.Cells[COLS.FromPostCode].Value.ToStr();
                //    else
                //        objBooking.FromAddress = grdBookings.CurrentRow.Cells[COLS.FromAddress].Value.ToStr();



                //    objBooking.FromDoorNo = grdBookings.CurrentRow.Cells[COLS.FromDoorNo].Value.ToStr();
                //    objBooking.FromStreet = grdBookings.CurrentRow.Cells[COLS.FromStreet].Value.ToStr();
                //    objBooking.FromPostCode = grdBookings.CurrentRow.Cells[COLS.FromPostCode].Value.ToStr();



                //    if (ToLocTypeId == Enums.LOCATION_TYPES.ADDRESS || ToLocTypeId == Enums.LOCATION_TYPES.BASE)
                //        objBooking.ToAddress = grdBookings.CurrentRow.Cells[COLS.ToAddress].Value.ToStr();
                ...
                //    objBooking.ToDoorNo = grdBookings.CurrentRow.Cells[COLS.ToDoorNo].Value.ToStr();
                //    objBooking.ToStreet = grdBookings.CurrentRow.Cells[COLS.ToStreet].Value.ToStr();
                //    objBooking.ToPostCode = grdBookings.CurrentRow.Cells[COLS.ToPostCode].Value.ToStr();

                //    objBooking.FareRate = grdBookings.CurrentRow.Cells[COLS.Fare].Value.ToDecimal();

                //    objBooking.PickupDateTime = grdBookings.CurrentRow.Cells[COLS.PickupDate].Value.ToDateTime();
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            objMaster.Current.CustomerMobileNo = txtMobileNo.Text.Trim();
                            objMaster.Current.CustomerPhoneNo = txtTelephoneNo.Text.Trim();
                            objMaster.Current.NoofLuggages = numTotalLuggages.Value.ToIntorNull();
                            objMaster.Current.NoofPassengers = num_TotalPassengers.Value.ToIntorNull();
                            objMaster.Current.CompanyId = ddlAccount.SelectedValue.ToIntorNull();
                            objMaster.Current.PaymentTypeId = ddlPaymentType.SelectedValue.ToInt();
                            objMaster.Current.IsCompanyWise = objMaster.Current.CompanyId != null;

                            objMaster.Current.VehicleTypeId = ddlVehicle.SelectedValue.ToIntorNull();
                            objMaster.Current.SpecialRequirements = txtSpecialReq.Text.Trim();
                            ...
                            if (grdBookings.Rows[i].Cells[COLS.MasterJobId].Value == null)
                            {
                                ...
                                if (chkPickupFares.Checked)
                                {
                                    objMaster.Current.FareRate = grdBookings.Rows[i].Cells[COLS.Fare].Value.ToDecimal();

                                }

                                if (chkCustomerPrice.Checked)
                                {
                                    objMaster.Current.CustomerPrice = numCustomerPrice.Value.ToDecimal();
                                }
                                if (chkCompanyPrice.Checked)
                                {
                                    objMaster.Current.CompanyPrice = numCompanyPrice.Value.ToDecimal();
                                }


                                if (chkReturnCompanyPrice.Checked)
                                {
                                    objMaster.Current.WaitingMins = numReturnCompanyPrice.Value.ToDecimal();
                                }
                                if (chkReturnCustomerPrice.Checked)
                                {
                                    objMaster.ReturnCustomerPrice = numReturnCustomerFare.Value.ToDecimal();
                                }


                                if (chkReturnFares.Checked)
                            ...
                            else
                            {
                                ...
                                if (chkReturnCustomerPrice.Checked)
                                {
                                    objMaster.Current.CustomerPrice = numReturnCustomerFare.Value.ToDecimal();
                                }
                                if (chkReturnCompanyPrice.Checked)
                                {
                                    objMaster.Current.CompanyPrice = numReturnCompanyPrice.Value.ToDecimal();
                                }


                                objMaster.Current.SpecialRequirements = txtReturnSpecialRequirement.Text.Trim();
                                    //if (chkReturnCompanyPrice.Checked)
                                    //{
                                    //    objMaster.Current.WaitingMins = numReturnCompanyPrice.Value.ToDecimal();
                                    //}
                            ...
                            objMaster.Current.BookingStatusId = grdBookings.Rows[i].Cells[COLS.BookingStatus].Value.ToIntorNull();
                            ...
                            if (grdVia != null && updateVia==true)
                            {
                                ...
                                else
                                {
                                    ...
                                    string[] skipProperties = { "Gen_Location", "Booking", "Gen_LocationType" };
                                    IList<Booking_ViaLocation> savedList = objMaster.Current.Booking_ViaLocations;
                                    List<Booking_ViaLocation> listofDetail = (from r in grdVia.Rows.OrderByDescending(c=>c.Index)
                                                                              select new Booking_ViaLocation
                                                                              {
                                                                                  Id = r.Cells["ID"].Value.ToLong(),
                                                                                  BookingId = r.Cells["MASTERID"].Value.ToLong(),
        ...
        private bool SendAdvanceBookingConfirmationText(string mobileNo)
        {
            ...
            if (enableAdvBookingText && listOfPickUpDateTime.Count > 0 && this.ObjBookiing != null && this.ObjBookiing.IsQuotation.ToBool() == false)
            {
                ...
                if (foundAny)
                {
                    ...
                    string refMsg = "";
                    rtn = General.SendAdvanceBookingSMS(mobileNo, ref refMsg, msg);
        ...
        private void CreateWeeklyBooking()
        {
            try
            {
                ...
                // dtpStartingCancel.Value = startingAt.ToDate();
            ...
            try
            {
                ...
                if (chkDayWise.Checked.ToBool() == false)
                {
                    ...
                    if (chkStartingAt.Checked && chkEndingAt.Checked)
                    {
                        ...
                        foreach (var gRow in grdBookings.Rows.Where(c=>c.Cells[COLS.StatusId].Value.ToInt()==Enums.BOOKINGSTATUS.WAITING
                                                 || c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.ONHOLD
                                                 || c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.PENDING).Where(c => c.Cells[COLS.EXCLUDED].Value.ToStr() == ""))
                        {
                    ...
                    else if (chkStartingAt.Checked && chkEndingAt.Checked == false)
                    {
                        ...
                        foreach (var gRow in grdBookings.Rows.Where(c => c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.WAITING
                                                 || c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.ONHOLD
                                                 || c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.PENDING).Where(c => c.Cells[COLS.EXCLUDED].Value.ToStr() == "" && c.Cells[COLS.PickupDate].Value.ToDate() <= endingAt))
                        {
                    ...
                    else if (chkStartingAt.Checked == false && chkEndingAt.Checked == true)
                    {
                        ...
                        foreach (var gRow in grdBookings.Rows.Where(c => c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.WAITING
                                                 || c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.ONHOLD
                                                 || c.Cells[COLS.StatusId].Value.ToInt() == Enums.BOOKINGSTATUS.PENDING).Where(c => c.Cells[COLS.EXCLUDED].Value.ToStr() == "" && c.Cells[COLS.PickupDate].Value.ToDate() >= startingAt))
                        {
        ...
        private void chkReturnFares_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            ...
            // numReturnFares.Enabled = args.ToggleState.ToBool();// == ToggleState.On;
            if (args.ToggleState == ToggleState.On)
            {
                numReturnFares.Enabled = true;
            }
            else
            {
                numReturnFares.Enabled = false;
            }
        ...
        public void UpdateCancelChanges()
        {
            ...
            try
            {
                for (int i = 0; i < grdBookings.Rows.Count; i++)
                {
                    ...
                    {
                        //BookingBO booking = new BookingBO();
                        if (chkCancelBooking.Checked == true)
                        {
                            objMaster = new BookingBO();
                            objMaster.GetByPrimaryKey(grdBookings.Rows[i].Cells[COLS.ID].Value.ToLong());
                            ...
                            if (grdBookings.Rows[i].Cells[COLS.BookingStatus].Value.ToIntorNull() == Enums.BOOKINGSTATUS.CANCELLED)
                            {
                                objMaster.Current.BookingStatusId = grdBookings.Rows[i].Cells[COLS.BookingStatus].Value.ToIntorNull();
                            }
                            objMaster.Save();
                        ...
                        else if (IsSuccess == false)
                        {
                            ...
                            if (grdBookings != null && grdBookings.Rows.Count > 0)
                            {
                                ...
                                if (grdBookings.Rows[i].Cells[COLS.BookingStatus].Value.ToIntorNull() == Enums.BOOKINGSTATUS.CANCELLED)
                                {
                                    ...
                                    foreach (var item in grdBookings.Rows)
                                    {
                                        //if(item.Cells[COLS.BookingStatus].Value.ToIntorNull() == Enums.BOOKINGSTATUS.CANCELLED)
                                        //{
                                    ...
                                    if (query.Length > 0)
                                    {
                                        using (TaxiDataContext db = new TaxiDataContext())
                                        {
                                            string FinalQuery = "update Booking set BookingStatusId=3 where Id in (" + query + ")";
                                            db.stp_UpdateCancelBooking(FinalQuery);
                                            IsSuccess = true;
                                        }
                                    }
                                    //string Idz = grdBookings.Rows.Select(c => c.Cells["ID"].Value.ToStr() &&(c.Cells[COLS.BookingStatus].Value.ToIntorNull() == Enums.BOOKINGSTATUS.CANCELLED).ToStr());
                                    //BookingId += string.Join(",", grdBookings.Rows.Select(c => c.Cells["ID"].Value.ToStr() && c.Cells["BookingStatus"].Value.ToIntorNull()==Enums.BOOKINGSTATUS.CANCELLED).ToArray<string>());
                                }
                            }
                            //if (grdBookings.Rows[i].Cells[COLS.BookingStatus].Value.ToIntorNull() == Enums.BOOKINGSTATUS.CANCELLED)
                            //{
        ...
        private void chkReturnCustomerPrice_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
                numReturnCustomerFare.Enabled = true;
            }
            else
            {
                numReturnCustomerFare.Enabled = false;
            }
        }
        ...
        private void chkReturnCompanyPrice_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            if (args.ToggleState == ToggleState.On)
            {
            ...
            //numReturnCompanyPrice.Enabled = args.ToggleState.ToBool();
        }
        ...
        private void ddlPaymentType_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            try
            {
                ...
                //}
                //if (ddlPaymentType.SelectedValue.ToInt() == Enums.PAYMENT_TYPES.CASH)
                //{
                //    if (ddlCompany.SelectedValue != null)
                //    {
                //        //if (objMaster.Current != null && objMaster.Current.CompanyId.ToInt() == ddlCompany.SelectedValue.ToInt() && objMaster.Current.Gen_Company.DefaultIfEmpty().AccountTypeId.ToInt() == Enums.ACCOUNT_TYPE.ACCOUNT)
                //        //{
                ...
                //            int accountTypeId = General.GetObject<Gen_Company>(c => c.Id == ddlCompany.SelectedValue.ToInt()).DefaultIfEmpty().AccountTypeId.ToInt();


                //            if (accountTypeId == Enums.ACCOUNT_TYPE.ACCOUNT)
                //            {
                //                ddlCompany.SelectedValue = null;
                //                //  chkIsCompanyRates.Checked = false;
                //            }
        ...
        private void ddlAccount_SelectedIndexChanged(object sender, Telerik.WinControls.UI.Data.PositionChangedEventArgs e)
        {
            try
            {
                ...
                if (accountTypeId == Enums.ACCOUNT_TYPE.CASH)
                {
                    //  ddlCompany.SelectedValue = null;
                    //  chkIsCompanyRates.Checked = false;
                    ddlPaymentType.SelectedValue = Enums.ACCOUNT_TYPE.ACCOUNT;
                }
                else
                {
