...
{
    public partial class frmDriverRentPayExpenses2 : UI.SetupBase
    {
        bool IsDriverLoaded = false;
        long RentId = 0;
        DriverRentBO objDriverRent = new DriverRentBO();
        public frmDriverRentPayExpenses2()
        {
            InitializeComponent();
            FormatGrid();
           ...
           //     rdoCredit.IsChecked = true;
                ...
                if (item != null)
                {
                  ...
                  //  ShowPaymentHistory(item.PayHistory.ToList());
                    ...
                    var list = (from a in General.GetQueryable<Fleet_DriverRentExpense>(c => c.RentId == RentId && (c.IsPaid == true))
                                //where a.IsPaid == true
                                select new
                                {
                                    Id = a.Id,
