        ...
        public void GenerateReport()
        {
            try
            {
               ...
               // string className = "Taxi_AppMain.ReportDesigns.";
                ...
                CashTotalAmount = this._DataSource.Where(c => c.PaymentTypeId == Enums.PAYMENT_TYPES.CASH).Sum(c=>c.FareRate + c.WaitingCharges  ).ToDecimal();
                AccountTotalAmount = this._DataSource.Where(c => c.PaymentTypeId == Enums.PAYMENT_TYPES.BANK_ACCOUNT).Sum(c => c.FareRate + c.WaitingCharges).ToDecimal();
                CardTotalAmount = this._DataSource.Where(c => (c.PaymentTypeId != Enums.PAYMENT_TYPES.CASH) && (c.PaymentTypeId != Enums.PAYMENT_TYPES.BANK_ACCOUNT)).Sum(c => c.FareRate + c.WaitingCharges).ToDecimal();
