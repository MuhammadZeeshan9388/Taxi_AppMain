...
{
    public partial class frmDriverCommissionDebitCredit2 : UI.SetupBase
    {
        ...
        public frmDriverCommissionDebitCredit2()
        {
            InitializeComponent();
            InitializeConstructor();

        }
        public frmDriverCommissionDebitCredit2(int Id)
        {
            InitializeComponent();
            InitializeConstructor();
        ...
        private void InitializeConstructor()
        {
            ...
            ComboFunctions.FillDriverNoCombo(ddlDriver, c => c.DriverTypeId == 2 && c.IsActive==true);
                ...
                if (driverId > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (obj != null)
                        {
                            ...
                            if (obj.Fleet_Driver_DebitCreditNotes.Count > 0)
                            {

                                numCollectionDeliveryAmount.Value = obj.Fleet_Driver_DebitCreditNotes[0].Charges.ToDecimal();
            ...
            try
            {
                ...
                if (obj != null)
                {
                    ...
                    else
                    {
                        ...
                        if (query != null && query.IsPaid.ToBool() == false)
                        {
                        //if (query != null && query.IsPaid.ToBool()==false && query.Balance.ToDecimal() >0) // comment on 19sept
                        //{
                            decimal driverBalance = query.Balance.ToDecimal()+ query.OldBalance.ToDecimal();
        ...
        private void OnSave()
        {
            try
            {
                ...
                objMaster.Current.TransactionType = Enums.TRANSACTIONTYPE.DRIVER_COMMISSION_EXPENSE2;
        ...
        private void CalculateTotal()
        {
           ...
           //     totalCredit = (commissionTotal + Credit + (spnAccountExpenses.Value.ToDecimal()) + numAgentFeeTotal.Value + numTotalCollectionDelivery.Value+numpdaRent.Value);
            //    totalDebit = (txtTotalAccountBooking.Text.TrimStart('£', ' ').ToDecimal() + Debit);
              ...
              //  decimal oldBlc = txtOldBalance.Text.TrimStart('£', ' ').Length > 0 ? txtOldBalance.Text.TrimStart('£', ' ').ToDecimal() : 0.00m;

                //on 19sept if old blc > 0 then add it to total
                //if (oldBlc < 0)
                //    oldBlc = 0.00m;

                Currentbalance = owedBalance ;
                //Currentbalance = owedBalance + oldBlc;
        ...
        public override void Print()
        {
            ...
            DockWindow doc = UI.MainMenuForm.MainMenuFrm.GetDockByName("frmDriverCommisionTransactionExpensesReport21");
