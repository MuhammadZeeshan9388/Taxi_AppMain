...
{
    public partial class frmCustomerInvoice_old : UI.SetupBase
    {
      ...
      //      grdLister.Columns["OrderNo"].Width = 55;
        ...
        private void btnPickBooking_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (filterBy == 0)
                {
                    ...
                    _condition=(c=>  c.CustomerName == customer && (telNo == string.Empty || c.CustomerPhoneNo == telNo) && (mobNo == string.Empty || c.CustomerMobileNo == mobNo)
                ...
                else if (filterBy == 1)
                {
                    _condition = (c => (c.CompanyId == null || (c.Gen_Company.SysGenId != null && c.Gen_Company.SysGenId == 2) || c.Gen_Company.AccountTypeId == Enums.ACCOUNT_TYPE.CASH)
                                                                                       && c.CustomerName == customer && (telNo == string.Empty || c.CustomerPhoneNo == telNo) && (mobNo == string.Empty || c.CustomerMobileNo == mobNo)
                                               && (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED));
        ...
        {
            var list1 = General.GetGeneralList<Booking>(_condition).Where(b => b.PickupDateTime.ToDate() >= fromDate && b.PickupDateTime.ToDate() <= tillDate);
       ...
       //     txtInvoiceAmount.Text =string.Format("{0:£ #.##}",grdLister.Rows.Sum(c => c.Cells[COLS.Total].Value.ToDecimal()));
        ...
        private void ddlCustomer_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (row != null)
                {
                    ...
                    this.EmailAddress = row.Cells["Email"].Value.ToStr();
                }
            }
        ...
        private void btnCustomerLister_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var list = General.GetQueryable<Customer>(c => c.Name != "" && (c.MobileNo == mobNo || mobNo == "") && (c.TelephoneNo == telNo || telNo == ""))
