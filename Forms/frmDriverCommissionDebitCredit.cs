...
{
    public partial class frmDriverCommissionDebitCredit : UI.SetupBase
    {
        ...
        public frmDriverCommissionDebitCredit()
        {
            InitializeComponent();
            InitializeConstructor();

        }
        public frmDriverCommissionDebitCredit(int Id)
        {
            InitializeComponent();
            InitializeConstructor();
        ...
        private void InitializeConstructor()
        {
          ...
          //  listofServiceCharges = General.GetServiceChargesList();
        }
        ...
        void btnExportPDF_Click(object sender, EventArgs e)
        {
            ...
            var list = General.GetQueryable<vu_DriverCommisionExpense>(a => a.Id == id).OrderBy(c => c.PickupDate).ToList();
        ...
        private void FormatChargesGrid()
        {
            ...
            colD.DecimalPlaces = 2;
            colD.Minimum = 0;
            colD.HeaderText = "Service Charge";
            colD.Name = COLS.ServiceCharges;
            colD.Maximum = 9999999;
            colD.IsVisible =false;
            colD.FormatString = "{0:#,###0.00}";
            grdLister.Columns.Add(colD);
            ...
            //colD.DecimalPlaces = 2;
            //colD.Minimum = 0;
            //colD.HeaderText = "ApplyServiceCharge";
            //colD.Name = COLS.ServiceCharges;
            //colD.Maximum = 9999999;
            //colD.IsVisible = false;
            //colD.FormatString = "{0:#,###0.00}";
        ...
        private void OnSave()
        {
            try
            {
             ...
             //   objMaster.Current.CommissionTotal = ((objMaster.Current.JobsTotal - objMaster.Current.AgentFeesTotal) * numCommissionPercent.Value) / 100;
                ...
                objMaster.Current.TotalServiceCharges = spnServiceCharges.Value.ToDecimal();
        ...
        public override void DisplayRecord()
        {
            ...
            spnServiceCharges.Value = objMaster.Current.TotalServiceCharges.ToDecimal();
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                if (objBooking != null)
                {
                    ...
                    grdLister.Rows[i].Cells[COLS.ServiceCharges].Value = objBooking.ServiceCharges.ToDecimal();
        ...
        private void CalculateTotal()
        {
            try
            {
                ...
                decimal pdaRent = numpdaRent.Value;
                   ...
                   txtTotalCashBooking.Text = string.Format("{0:£ #.##}", grdLister.Rows.Where(c => c.Cells[COLS.PaymentTypeId].Value.ToInt()==Enums.PAYMENT_TYPES.CASH).Sum(c => c.Cells[COLS.Fares].Value.ToDecimal()));


                // change from Total to Fare Sum
                txtTotalAccountBooking.Text = string.Format("{0:£ #.##}", grdLister.Rows.Where(c =>  c.Cells[COLS.PaymentTypeId].Value.ToInt() != Enums.PAYMENT_TYPES.CASH).Sum(c => c.Cells[COLS.Fares].Value.ToDecimal()));
                ...
                spnServiceCharges.Value = grdLister.Rows.Where(c => c.Cells[COLS.PaymentTypeId].Value.ToInt() == Enums.PAYMENT_TYPES.CASH)
                                        .Sum(c => Math.Round(c.Cells[COLS.ServiceCharges].Value.ToDecimal(), 2));
                ...
                totalDebit = (numpdaRent.Value + txtCommsionTotal.Text.ToDecimal() + Debit + spnServiceCharges.Value);
        ...
        public override void Print()
        {
            ...
            DockWindow doc = UI.MainMenuForm.MainMenuFrm.GetDockByName("frmDriverCommisionTransactionExpensesReport1");
        ...
        private void btnPick_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!IsFareAndWaitingWiseComm)
                    hiddenColumns = new string[] {  "Id", "Parking","Destination","ExtraDrop","MeetAndGreet","Congtion",
                                                "Total","OrderNo","PupilNo","BookingDate","Description","Charges","AccountType","CompanyId","Waiting"
                                                ,"IsCommissionWise","DriverCommissionType","DriverCommission","ServiceCharges","ApplyServiceCharge","CustomerPrice","PaymentTypeId","BookedBy","AgentCommission"};
                ...
                List<object[]> list = General.ShowDriverCommTransactionExpenseBookingMultiLister(c => c.DriverId == DriverId && (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED), hiddenColumns, _conditionDate);
              ...
              //  GridViewRowInfo row;
                ...
                //    objExisting[31] = item.Cells[COLS.ServiceCharges].Value.ToInt();
                ...
                for (int i = 0; i < totalRows; i++)
                {
                    ...
                    if (list[i][27] != null && list[i][11].ToIntorNull()>0 &&  list[i][22].ToInt()!=Enums.PAYMENT_TYPES.CASH) 
                    {
                        a1 += list[i][27].ToDecimal();
                    }
                    if (list[i][28] != null && list[i][11].ToIntorNull() > 0 && list[i][22].ToInt() != Enums.PAYMENT_TYPES.CASH)
                    {
                    ...
                    grdLister.Rows[i].Cells[COLS.ServiceCharges].Value = list[i][31].ToDecimal();
