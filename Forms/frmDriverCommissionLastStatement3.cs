...
{
    public partial class frmDriverCommissionLastStatement3 : UI.SetupBase
    {
        private bool IsFareAndWaitingWiseComm;
        public frmDriverCommissionLastStatement3()
        {
            InitializeComponent();
            FormatGrid();
            ...
            try
            {
                if (grdDriverRentLastStatement.Rows.Where(c => c.Cells["Check"].Value.ToBool() == true).Count() > 0)
                {
                    ...
                    //}
                        ...
                        if (list.Count > 0)
                        {
                            frmDriverCommisionTransactionExpensesReport3 frm = new frmDriverCommisionTransactionExpensesReport3(list, DateTime.Now.ToDate(), DateTime.Now.ToDate());
                            frm.ShowDialog();
                            frm.Dispose();
                        }                   
