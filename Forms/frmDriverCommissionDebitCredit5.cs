...
namespace Taxi_AppMain
{
    public partial class frmDriverCommissionDebitCredit5 : UI.SetupBase
    {
                ...
                if (gridCell.ColumnInfo.Name == "btnUpdate")
                {
                    ...
                    if (row is GridViewDataRowInfo)
                    {
                        ...
                        if (objMaster.Current != null)
                        {
                            ...
                            row.Cells[COLS.Total].Value = (fare  + waiting ) - bookingfee;

                            row.Cells[COLS.Commission].Value = (row.Cells[COLS.Total].Value.ToDecimal() * numCommissionPercent.Value) / 100;
        ...
        private void FormatChargesGrid()
        {
            ...
            //{
                CommissionExpression = "((Fares+Waiting)-BookingFees)";
        ...
        private void CalculateTotal()
        {
            try
            {
                ...
                if (numVAT.Value > 0)
                {
                    commissionTotal = commissionTotal + Math.Round( ((commissionTotal * numVAT.Value) / 100));
           ...
           //     totalCredit = (commissionTotal + Credit + (spnAccountExpenses.Value.ToDecimal()) + numAgentFeeTotal.Value + numTotalCollectionDelivery.Value+numpdaRent.Value);
            //    totalDebit = (txtTotalAccountBooking.Text.TrimStart('£', ' ').ToDecimal() + Debit);
                ...
                totalCredit = (txtTotalAccountBooking.Text.TrimStart('£', ' ').ToDecimal() + spnAccountExpenses.Value.ToDecimal() + parkingTotal+ extraTotal+ Credit);
            ...
            try
            {
                ...
                if (!IsFareAndWaitingWiseComm)
                    hiddenColumns = new string[] {  "Id", "Parking","Destination","ExtraDrop","MeetAndGreet","Congtion",
                                                ...
                                                ,"IsCommissionWise","DriverCommissionType","DriverCommission","PaymentTypeId","DropOffCharge","PickupCharge"};
