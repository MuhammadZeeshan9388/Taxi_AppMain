...
{
    public partial class frmDriverRentDebitCredit2 : UI.SetupBase
    {
        ...
        public frmDriverRentDebitCredit2()
        {
            InitializeComponent();
        ...
        public frmDriverRentDebitCredit2(int Id)
        {
            InitializeComponent();
            InitializeConstructor();
        ...
        public frmDriverRentDebitCredit2(long Id, bool IsHide)
        {
            InitializeComponent();
            InitializeConstructor();
            ddlDriver.SelectedValue = Id;
...
//            grdLister.CommandCellClick+=new CommandCellClickEventHandler(grdLister_CommandCellClick);
        ...
        protected override void OnClosed(EventArgs e)
        {
            ...
            General.RefreshListWithoutSelected<frmDriverRentList2>("frmDriverRentList21");
        ...
        private void btnPickBooking_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //    _conditionDate = b => b.BookingDate.Value.Date >= fromDate && b.BookingDate.Value.Date <= tillDate;
                    ...
                    List<object[]> list = General.ShowDriverRentTransactionExpensesWithWaitingBookingMultiLister(c => c.DriverId == DriverId && (c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED)  
             ...
             //   list.RemoveAll(c => existBookingId.Contains(c[0].ToLong()));
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    grdLister.Rows[i].Cells[COLS.Parking].Value = list[i][16].ToDecimal();
                    grdLister.Rows[i].Cells[COLS.Waiting].Value = list[i][17].ToDecimal();
                    // grdLister.Rows[i].Cells[COLS.ExtraDrop].Value = list[i][15].ToDecimal();


                    //   grdLister.Rows[i].Cells[COLS.MeetAndGreet].Value = list[i][16].ToDecimal();    //waiting
                    //   grdLister.Rows[i].Cells[COLS.CongtionCharge].Value = list[i][17].ToDecimal();  //parking

                    // old : 7 aug 15
                    //grdLister.Rows[i].Cells[COLS.Total].Value = list[i][21].ToDecimal();
                    //new : 7 aug 15
        ...
        public override void Print()
        {
            ...
            DockWindow doc = UI.MainMenuForm.MainMenuFrm.GetDockByName("frmDriverTransactionExpensesReport21");
            if (doc != null)
            {
                doc.Close();
            }
