...
{
    public partial class frmAddAllDriverRentExpenses : UI.SetupBase
    {
        ...
        public frmAddAllDriverRentExpenses()
        {
          ...
          //  decimal Debit = 0.00m;
            ...
            try
            {
              ...
              bool RentForProcessedJobs = AppVars.objPolicyConfiguration.RentForProcessedJobs.ToBool();
                ...
                var list2 = (from a in General.GetGeneralList<Booking>(c => c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED && c.PaymentTypeId!=Enums.PAYMENT_TYPES.CASH
                    && (c.FareRate != null)
                ...
                else
                {
                    ...
                    totalDebit = (DriverRent + TotalpdaRent + TotalCarRent + TotalCarInsuranceRent + TotalPrimeCompanyRent + Debit);
                    ...
                    objMaster.Current.TransactionType = Enums.TRANSACTIONTYPE.DRIVER_RENT_EXPENSE1;
                    objMaster.Save();
        ...
        private void btnViewPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    frmDriverRentTransactionExpensesReport frm = new frmDriverRentTransactionExpensesReport(list, dtpFromDate.Value.ToDate(), dtpTillDate.Value.ToDate());
                    frm.ShowDialog();
                    frm.Dispose();
                }
            }
