...
{
    public partial class frmAddAllDriverCommissionExpenses2 : UI.SetupBase
    {
        ...
        public frmAddAllDriverCommissionExpenses2()
        {
        ...
        private void RefreshCommissionView()
        {
            try
            {
                ...
                {
                    var list = db.stp_AddAllDriverCommission(dtFrom, dtTill).AsEnumerable().OrderBy(item => item.Driver, new NaturalSortComparer<string>()).ToList();
            ...
            try
            {
                ...
                else
                {
                    ...
                    //{
                        ...
                        if (query != null)
                        {
                            if (query.IsPaid.ToBool() == false)
                            {
                                //if (query != null && query.IsPaid.ToBool() == false && query.Balance.ToDecimal()> 0)//comment on 19sept16
                                //{
                                oldBalance = query.Balance.ToDecimal();
                            }
                            else
                                oldBalance = 0.00m;
                        }
              ...
              //{
                    ...
                    //if (DriverCommissionTotal > 0)
                    //    DriverCommissionTotal = (decimal)(Math.Ceiling(Convert.ToDouble(DriverCommissionTotal) / 0.25) * 0.25);
                    ...
                    objMaster.Current.Balance = (owed);
        ...
        private void btnViewPrint_Click(object sender, EventArgs e)
        {
                ...
                try
                {
                  ...
                  //  List<long> invoiceIds = rows.Select(c => c.Cells[COLS.CommissionId].Value.ToLong()).ToList<long>();
                    ...
                    if (list.Count > 0)
                    {
                        frmDriverCommisionTransactionExpensesReport2 frm = new frmDriverCommisionTransactionExpensesReport2(list,dtpFromDate.Value.ToDate(),dtpTillDate.Value.ToDate());
                        frm.ShowDialog();
                        frm.Dispose();
                    }
                }
