...
{
    public partial class frmDriverCommissionLastStatement2 : UI.SetupBase
    {
        private bool IsFareAndWaitingWiseComm;
        public frmDriverCommissionLastStatement2()
        {
            InitializeComponent();
            FormatGrid();
        ...
        private void PrintStatement()
        {
            try
            {
                ...
                if (grdDriverRentLastStatement.Rows.Where(c => c.Cells["Check"].Value.ToBool() == true).Count() > 0)
                {
                    ...
                    if (list.Count > 0)
                    {
                        frmDriverCommisionTransactionExpensesReport2 frm = new frmDriverCommisionTransactionExpensesReport2(list, DateTime.Now.ToDate(), DateTime.Now.ToDate());
                        frm.ShowDialog();
                        frm.Dispose();
                    }
