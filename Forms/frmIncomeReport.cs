...
{
    public partial class frmIncomeReport : UI.SetupBase
    {
        public struct COLS
        {
            ...
            try
            {
                ...
                var query = General.GetQueryable<Booking>(c => c.BookingStatusId == Enums.BOOKINGSTATUS.DISPATCHED
                               // && (c.SubcompanyId==AppVars.DefaultBookingSubCompanyId || AppVars.DefaultBookingSubCompanyId==0)
                                ...
                                &&(c.DriverId==driverId || driverId==0) && (c.CompanyId==companyId || companyId==0)
                                &&(c.SubcompanyId==SubCompanyId || SubCompanyId==0)
                                && (c.FleetMasterId==FleetMasterId || FleetMasterId==0)
                                    );
                ...
                var list = (from a in query
                            ...
                            //    Destination = !string.IsNullOrEmpty(a.ToDoorNo) ? a.ToDoorNo + " - " + a.ToAddress : a.ToAddress,
                                ...
                                a.DriverId,
                                Comm=  a.Fleet_Driver.DriverCommissionPerBooking
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    row.Cells[COLS.Via].Value = list[i].Via;
                    //Via
                    ...
                    row.Cells[COLS.DriverId].Value = list[i].DriverId.ToDecimal();
                    row.Cells[COLS.Comm].Value = list[i].Comm.ToDecimal();
                ...
                string total50PerEarning = list.Where(c => c.Comm != null && c.Comm == 50).Sum(c => c.Total).ToString();
				lblTotal50PerEarning.Text = "Total 50% £ " + total50PerEarning;

				string total15PerEarning = list.Where(c => c.Comm != null && c.Comm == 15).Sum(c => c.Total).ToString();
                lblTotal15PerEarning.Text = "Total 15% £ " + total15PerEarning;

                string total100PerEarning = list.Where(c => c.Comm != null && c.Comm == 100).Sum(c => c.Total).ToString();
                lblTotal100Earning.Text = "Total 100% £ " + total100PerEarning;
        ...
        private List<Vu_BookingBase> GetDataSource(DateTime? fromDate, DateTime? tillDate, int companyId, int driverId, int SubCompanyId, long FleetMasterId)
        {
