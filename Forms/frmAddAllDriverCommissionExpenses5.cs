...
namespace Taxi_AppMain
{
    public partial class frmAddAllDriverCommissionExpenses5 : UI.SetupBase
    {
        ...
        private void Generate()
        {
            try
            {
                ...
                foreach (var row in grdDriverCommission.Rows.Where(c => c.Cells["Check"].Value.ToBool()))
                {
                    ...
                    //MaxCommission
                    MaxCommission = listofDrivers.Where(c => c.Id == row.Cells["Id"].Value.ToInt()).FirstOrDefault().MaxCommission.ToDecimal();
                    //
                    ...
                    if (OnSave(row.Cells["Id"].Value.ToInt(), row.Cells[COLS.OldBalance].Value.ToDecimal(), row.Cells[COLS.DriverCommissionPerBooking].Value.ToDecimal()
                        ...
                        , ref accJobsTotal, ref transId, ref rentDue, ref currBalance, row.Cells[COLS.Hokiday].Value.ToBool(), ref MaxCommission, row.Cells[COLS.VAT].Value.ToDecimal()))
                    {
        ...
        private bool OnSave(int DriverId, decimal oldBalance, decimal DriverCommissionPerBooking, decimal pdaCommission, decimal InitialBalance, decimal CommissionPayValue, ref decimal accJobs, ref long transId, ref decimal rentDue, ref decimal currBalance, bool IsHoliday, ref decimal MaxCommission, decimal VAT)
        {
            ...
            try
            {
                ...
                var list2 = (from a in General.GetGeneralList<Booking>(_condition)
                             ...
                             select new
                             {
                                 ...
                                 DropOffCharge = 0,
                                 PickupCharge = 0,
                                 Parking = a.CongtionCharges.ToDecimal(),
                                 BookingFee = a.ServiceCharges.ToDecimal(),
                                 a.ExtraDropCharges
                             }).ToList();
                ...
                if(VAT.ToDecimal()>0)
                {
                    DriverCommissionTotal= DriverCommissionTotal+((DriverCommissionTotal*VAT.ToDecimal())/100);
                ...
                totalCredit = (ACCJobsTotal + parkingTotal + AccountExpenses + Credit);
            ...
            try
            {
                ...
                foreach (var row in grdDriverCommission.Rows.Where(c => c.Cells["Check"].Value.ToBool()))
                {
                    ...
                    try
                    {
                        ...
                        //decimal Total = list2.Sum(c => (c.TotalFare)).ToDecimal();
                        decimal Total = list2.Sum(c => (c.TotalFare) + (c.WaitingCharges)).ToDecimal();

                        //decimal ACCJobsTotal = list2.Where(c =>  c.AccountTypeId == Enums.ACCOUNT_TYPE.ACCOUNT).Sum(c => c.TotalFare).ToDecimal();
                        decimal ACCJobsTotal = list2.Where(c => c.AccountTypeId == Enums.ACCOUNT_TYPE.ACCOUNT).Sum(c => (c.TotalFare) + (c.WaitingCharges)).ToDecimal();


                        decimal parkingTotal = list2.Sum(c => c.ParkingCharges.ToDecimal()).ToDecimal();


                        //decimal CashJobsTotal = list2.Where(c => c.CompanyId == null || c.AccountTypeId == Enums.ACCOUNT_TYPE.CASH).Sum(c => c.TotalFare).ToDecimal();
                        decimal CashJobsTotal = list2.Where(c => c.CompanyId == null || c.AccountTypeId == Enums.ACCOUNT_TYPE.CASH).Sum(c => (c.TotalFare) + (c.WaitingCharges)).ToDecimal();
